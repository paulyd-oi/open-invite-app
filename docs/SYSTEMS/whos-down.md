...
- **Unseen badge is local-only.** Proof tag: `[WHOS_DOWN_UNSEEN_BADGE_V1]`. SSOT: `src/lib/whosDownSeen.ts` (`useWhosDownSeen(userId)` hook, AsyncStorage key `whosDownSeen_<userId>`). The Discover Who's Down tab badge renders `whosDownUnseenCount` = `whosDownItems.filter(i => !seenIds.has(i.id)).length`, NOT `whosDownData.activeCount`. Seen IDs are persisted per-user as a `string[]` with a soft cap (trim to 1000 when >2000). Marking rules: whenever `lens === "whos_down"` AND the hook is hydrated AND the feed has at least one item, the currently loaded IDs are marked seen via `markSeen(...)`. The effect re-runs when the feed changes, so mid-session refetches (post, respond, pull-to-refresh) mark late-arriving items seen as long as the user is still on the tab. The user's own posts get marked seen the same way because post success invalidates `qk.whosDownFeed()` while they are still on the Who's Down tab. No backend read-tracking, no per-item read state, no activity-based re-badging. Stale seen IDs for expired ideas are harmless (they simply no longer appear in the feed). On app relaunch the hook rehydrates from AsyncStorage.
...
Discover tabs are now **Map / Events / Who's Down** (`type Lens = "map" | "events" | "whos_down"`). The former "Responded" lens folded into Events as a `"responded"` pill with Going / Not Going sub-filter. The Who's Down lens label shows a count badge `(N)` when `whosDownUnseenCount > 0` — this is **unseen** ideas (new since the user last opened the tab), not total active ideas. Overflow clamps to `99+`. See the "Unseen badge is local-only" invariant below.
