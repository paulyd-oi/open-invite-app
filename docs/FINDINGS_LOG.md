...
### Finding
ios/ directory contains hand-maintained native product code that is NOT generated by `expo prebuild`. Running `expo prebuild --clean` DELETES these files irrecoverably from the working tree.

### Files at Risk
- ios/OpenInvite/LiveActivityBridge.m — ObjC bridge for RCT_EXTERN_MODULE (Live Activity)
- ios/OpenInvite/LiveActivityBridge.swift — React Native native module for ActivityKit Live Activities (start/update/end/getActive)
- ios/OpenInvite/WidgetBridge.m — ObjC bridge for RCT_EXTERN_MODULE (Widget data)
- ios/OpenInvite/WidgetBridge.swift — Native bridge writing today-events JSON to App Group shared UserDefaults for WidgetKit
- ios/OpenInviteTodayWidget/OpenInviteTodayWidget.swift — WidgetKit lock screen widget (accessoryRectangular family, timeline provider, SwiftUI view)
- ios/OpenInviteTodayWidget/OpenInviteLiveActivity.swift — Live Activity V2 UI (Lock Screen + Dynamic Island, ActivityKit, OS-driven countdown timer)
- ios/OpenInviteTodayWidget/Info.plist — Widget extension Info.plist (NSSupportsLiveActivities, version 1.0.9/263)
- ios/OpenInviteTodayWidget/Assets.xcassets/ — Widget assets
- ios/OpenInviteTodayWidgetExtension.entitlements — App Group entitlement (group.com.vibecode.openinvite.0qi5wk)
- ios/Shared/OpenInviteEventAttributes.swift — ActivityKit attributes struct shared between main app and widget extension targets
- ios/OpenInvite.xcodeproj/project.pbxproj — Contains all target definitions, build phases, and file references for widget extension

### Evidence
- .gitignore line 40 comment: "generated native folders (allow tracked ios/ files for widget + native bridge)" — confirms ios/ is intentionally tracked
- JS callers exist: src/lib/liveActivity.ts (NativeModules.LiveActivityBridge), src/lib/widgetBridge.ts (NativeModules.WidgetBridge)
- Xcode project has OpenInviteTodayWidgetExtension as separate native target with its own build config, entitlements, and bundle ID
- Widget extension is NOT managed by any Expo config plugin — no plugin in app.json references widgets
- `expo prebuild --clean` regenerates ios/ from app.json + plugins — anything not generated by a plugin is destroyed

### Safe Hermes Switch Path (no clean prebuild)
Three config files need updating:
1. ios/Podfile.properties.json: `"expo.jsEngine": "jsc"` → `"hermes"` (this is what the Podfile reads at line 39)
2. ios/OpenInvite.xcodeproj/project.pbxproj: `USE_HERMES = false` → `USE_HERMES = true` (Debug config, line 627)
3. ios/OpenInvite.xcodeproj/project.pbxproj: `USE_HERMES = false` → `USE_HERMES = true` (Release config, line 684)
Then run: `cd ios && pod install` to swap React-jsc pods for hermes-engine pods.
No clean prebuild. No file deletion. No code changes.

### Recommendation
Do NOT run `expo prebuild --clean` on this repo until a native file preservation strategy is in place (either an Expo config plugin that regenerates widget/bridge files, or a documented manual restore process). The Hermes switch is safe via the 3-config-file approach above.
