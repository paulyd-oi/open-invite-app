...
## Stable
- Onboarding Photo Upload Fix V2: NEEDS REPRO — diagnostics added. Upload code path (uploadByKind) is identical between Settings and Onboarding (both call uploadImage→uploadByKind("avatar")). V1's network gate fix is confirmed merged. Cannot determine root cause from code analysis alone — the pipeline code, auth state, and image picker calls are the same. Added on-screen DEV diagnostic ([P0_PHOTO_UPLOAD] tag) that displays the exact failure step and error message as visible text in the onboarding UI (not just console.log). Also preserved error.status through uploadByKind's outer catch so caller diagnostics can read HTTP status codes. Apple Sign-In diagnostics button confirmed already DEV-gated (__DEV__ && Platform.OS === "ios"). TEMP DIAGNOSTICS KEPT — need one repro from Paul to identify the exact failure step.
- Onboarding Photo Upload Fix V1: Fixed photo upload failing during onboarding for accounts created after a prior logout. Root cause: the network auth gate (networkAuthGate.ts) was disabled by performLogout() but only re-enabled when bootStatus became 'authed'. During onboarding, bootStatus is 'onboarding', so the gate stayed closed — all api.post() calls including /api/uploads/sign were blocked with [P0_POST_LOGOUT_NET]. Fix: (1) useBootAuthority.ts now enables network for 'onboarding' status too, (2) exactAppleAuthBootstrap.ts calls enableAuthedNetwork() immediately after session barrier passes (closes race window between auth and photo pick), (3) networkAuthGate lifecycle comment updated to reflect new behavior.
- Onboarding Profile Setup Fix V1: Three bugs fixed on "Set Up Your Profile" screen. (A) Full name no longer auto-populates from email local-part — removed deriveDisplayNameFromEmail function, signup now passes no name to better-auth, prefill guard checks isNameDerivedFromEmail before populating displayName. (B) Profile save error path now logs full status + response body under [P0_PROFILE_SETUP] DEV tag for diagnosis. (C) Failed photo upload now clears avatarLocalUri (was leaving stale preview after upload failure).
