...
## Stable
- Onboarding Avatar Upload V2 Diagnostics: [ONBOARD_AVATAR] DEV-only diagnostic logs added to welcome.tsx (handlePickPhoto, uploadPhotoInBackground) and imageUpload.ts (uploadByKind). Logs cover: picker permission/result, session check, sign request/response, Cloudinary POST/response, complete request/response, and failure details (status, message, data). Investigation confirmed the UX Refresh V1 merge (97851db) provably did NOT change any upload, auth, or network code — the regression is not a code regression. Diagnostics needed to identify whether failure is backend-side (sign endpoint 403) or environment-side (simulator permissions, account state). TEMP DIAGNOSTICS — remove after root cause confirmed.
