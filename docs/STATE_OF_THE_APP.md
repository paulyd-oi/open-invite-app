...
## Stable
- Cache Invalidation Gaps V1: Fixed 5 cross-surface invalidation gaps from surface audit. feedPopular() added to all 5 event invalidation helpers. Discover save wired to SSOT. Friend accept now invalidates event feeds + notifications. Friend reject now invalidates notifications. Social feed now filters past/ended events.
...
- Onboarding Photo Upload RESOLVED: Root cause was backend returning 403 "Email not verified" on /api/uploads/sign for authenticated-but-unverified users. Fixed server-side. Frontend fixes retained: (1) network auth gate enables for 'onboarding' status (V1), (2) enableAuthedNetwork() called after session barrier (V1), (3) error.status/data preserved on uploadByKind re-throw for better error visibility. All temporary P0 diagnostics removed (V3 cleanup).
- Onboarding Profile Setup Fix V1: Three bugs fixed on "Set Up Your Profile" screen. (A) Full name no longer auto-populates from email local-part — removed deriveDisplayNameFromEmail function, signup now passes no name to better-auth, prefill guard checks isNameDerivedFromEmail before populating displayName. (B) Failed photo upload now clears avatarLocalUri (was leaving stale preview after upload failure).
- Event Detail Hook Order Fix V1: Fixed "Rendered more hooks than during the previous render" crash caused by Live Activity auto-start useFocusEffect being placed below early returns (loading gate, error gate, busy gate). Merged auto-start logic into the existing useFocusEffect at line ~544 (unconditional hook zone). Uses liveActivityAutoStartRef (updated every render) to pass derived values (event, isMyEvent, myRsvpStatus, effectiveGoingCount) to the callback without TDZ issues. Hook count now stable across all render paths.
- Live Activity Premium UI V1: Lock Screen redesigned from basic system row to premium themed card. Dark gradient background with theme accent tint (top-leading to bottom-trailing). Status pill with glass material + accent border shows OS-driven countdown timer or "Happening now". Title upgraded to .title3 bold rounded. Secondary metadata line with mappin icon + location + person.2.fill + going count. Emoji badge moved to top-right with accent-tinted circle. Dynamic Island expanded now shows location under countdown, going count as accent-colored number in trailing region. New themeAccentColor field added to ActivityKit payload (OpenInviteEventAttributes) — hex string from resolveEventTheme().backAccent, parsed to SwiftUI Color with #FF6B4A fallback. Bridge extended: LiveActivityBridge.swift + .m add themeAccentColor parameter. Plan B (themed gradient, no remote image) used per spec — ActivityKit 4KB payload limit makes image transfer impractical.
- Live Activity Auto-On V1: Live Activities now auto-start silently when viewing an event detail screen, if: (1) user is host or RSVP'd going, (2) event starts within 60 minutes or is ongoing, (3) no other Live Activity is active, (4) user hasn't manually dismissed this session. Manual toggle preserved in event actions sheet. Manual dismiss sets session-scoped ref to prevent re-auto-start; manual re-enable clears it. RSVP-triggered auto-start (4h horizon) unchanged. No native file changes — JS/TS only. New export `isEligibleForAutoStartOnFocus` in src/lib/liveActivity.ts with 60-minute window.
...
- Social tab swipe-vs-tap disambiguation V1: EventCard tracks touch movement via onTouchStart/onTouchMove. If finger moves >10px in any direction, wasSwiping flag suppresses onPress. Pan gesture onStart also sets flag. Prevents accidental event detail navigation during scroll or swipe-to-reveal. DEV proof: [SOCIAL_GESTURE]
