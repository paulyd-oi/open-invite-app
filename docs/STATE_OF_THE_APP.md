...
## Stable
- Onboarding Avatar Upload V2 FIXED: Root cause identified and fixed — `isValidBetterAuthToken()` in authSessionToken.ts incorrectly required a dot in session tokens (assumed JWT format), but Better Auth response body tokens are plain 32-char alphanumeric strings (e.g. "uGqE2i5o6IWl5X5m5RJJpn9imfdzOr4C"). This caused `runExactAppleAuthBootstrap()` to fail token validation after signup/signin, preventing session credentials (cookie + x-oi-session-token) from being stored. The fallback path (`captureAndStoreCookie`) did NOT set oiSessionToken, leaving the upload sign request without auth headers. Fix: removed dot requirement from `isValidBetterAuthToken()`, fixed `formatReactNativeCookieHeader()` which was producing malformed cookie headers with leading "; ". All [ONBOARD_AVATAR] diagnostic logs removed.
