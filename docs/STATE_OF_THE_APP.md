...
- Event Detail Canvas V1: Full-bleed themed canvas on event detail page. SafeAreaView top edge removed so canvas extends under status bar. Atmospheric zone bg set to canvasColor (theme backBgDark/backBgLight or system bg for unthemed). Nav bar manually padded with insets.top for notch safety. StatusBar style adapts to dark/light mode. Atmospheric gradients fade to canvasColor instead of system bg. 60px transition gradient below card fades canvas to system bg. Flip card and plaque preserved exactly. No new tokens. Discover not touched.
