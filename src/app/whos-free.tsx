      ...
      >
          ...
          <Animated.View entering={FadeIn.duration(200)} className="mb-4">
            ...
            >
              ...
              <View className="flex-row mb-3">
                {selectedFriends.slice(0, 5).map((friend, idx) => (
                  <View
                  ...
                  >
                    {friend.friend.image ? (
                      <Image source={{ uri: friend.friend.image }} className="w-full h-full" />
                    ) : (
                      <View
        ...
        <Animated.View entering={FadeInDown.springify()} className="mb-4">
          ...
          <Text style={{ color: colors.textSecondary }}>
            {selectedFriendIds.size > 0
              ? `Tap friends to add/remove from group • ${selectedFriendIds.size} selected`
              : "Tap friends to plan together"
            }
          </Text>
        </Animated.View>
          ...
          <>
            ...
            <Animated.View entering={FadeInDown.delay(50).springify()} className="mb-6">
              <View className="flex-row items-center mb-3">
                ...
                <Text className="text-base font-semibold" style={{ color: colors.text }}>
                  Free ({filteredFreeFriends.length})
                </Text>
              </View>

              {filteredFreeFriends.length === 0 ? (
                <View
                  className="rounded-xl p-6 items-center"
                  style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1 }}
                >
                  <Text className="text-center" style={{ color: colors.textSecondary }}>
                    No friends are free on this day
                  </Text>
                </View>
              ) : (
                <View className="space-y-2">
                  {filteredFreeFriends.map((item, index) => {
                    const isSelected = selectedFriendIds.has(item.friendshipId);
                    return (
                      <Pressable
                        key={item.friendshipId}
                        onPress={() => handleToggleFriend(item.friendshipId)}
                        onLongPress={() => handleFriendPress(item.friendshipId)}
                      ...
                      >
                        ...
                        <View
                          ...
                          style={{ backgroundColor: isSelected ? themeColor : "#22C55E" }}
                        >
                          {isSelected ? (
            ...
            <Animated.View entering={FadeInDown.delay(100).springify()} className="mb-6">
              <View className="flex-row items-center mb-3">
                ...
                <Text className="text-base font-semibold" style={{ color: colors.text }}>
                  Busy ({filteredBusyFriends.length})
                </Text>
              </View>

              {filteredBusyFriends.length === 0 ? (
                <View
                  className="rounded-xl p-4 items-center"
                  style={{ backgroundColor: colors.surface, borderColor: colors.border, borderWidth: 1 }}
                >
                  ...
                  {filteredBusyFriends.map((item) => {
                    ...
                    return (
                      <Pressable
                        ...
                        style={{
                          backgroundColor: isSelected ? themeColor + "10" : colors.surface,
                          borderColor: isSelected ? themeColor + "50" : colors.border,
                          borderWidth: isSelected ? 2 : 1,
                      ...
                      >
                        ...
                        >
                          {item.friend.image ? (
                            ...
                            <View
                              className="w-full h-full items-center justify-center"
                              style={{ backgroundColor: colors.textTertiary + "30" }}
                            >
                              <Text className="text-sm font-bold" style={{ color: colors.textTertiary }}>
                                {item.friend.name?.[0] ?? "?"}
                              </Text>
                            </View>
                          )}
                        ...
                        <View className="flex-1">
                          ...
                          <View className="flex-row items-center">
                            {item.isWorking ? (
                              <>
                                <Briefcase size={10} color={colors.textTertiary} />
                                <Text className="text-xs ml-1" style={{ color: colors.textTertiary }}>
                                  {item.workLabel ?? "Working"}
                                </Text>
                              </>
                            ) : (
                              <Text className="text-xs" style={{ color: colors.textTertiary }}>
            ...
            <Animated.View entering={FadeInDown.delay(150).springify()}>
              ...
              >
                ...
                <Text className="text-white font-semibold ml-2">
                  {selectedFriendIds.size > 0
                    ? `Create Event with ${selectedFriendIds.size} Friend${selectedFriendIds.size > 1 ? "s" : ""}`
                    : `Create Event for ${formattedDate.split(",")[0]}`
                  }
                </Text>
              </Pressable>
            </Animated.View>
      ...
      >
        <SafeAreaView className="flex-1" style={{ backgroundColor: colors.background }}>
          ...
          <View className="flex-row items-center justify-between px-4 py-3 border-b" style={{ borderBottomColor: colors.separator }}>
            ...
            <Text className="text-lg font-semibold" style={{ color: colors.text }}>Filters</Text>
            <Pressable
              onPress={() => {
                setTimeRange("all");
            ...
            >
              <Text style={{ color: themeColor }} className="font-medium">Reset</Text>
            </Pressable>
          </View>
          ...
          <ScrollView className="flex-1 p-4">
            {/* Time Range Filter */}
            <View className="mb-6">
              <Text className="text-sm font-medium mb-3" style={{ color: colors.textSecondary }}>TIME RANGE</Text>
              <View className="flex-row flex-wrap gap-2">
                {[
                  { id: "all", label: "Any Time" },
                  { id: "after_work", label: "After Work (5pm+)" },
                  ...
                  <Pressable
                    ...
                    style={{
                      backgroundColor: timeRange === option.id ? `${themeColor}20` : colors.surface,
                      borderWidth: 1,
                      borderColor: timeRange === option.id ? themeColor : colors.border,
                    }}
                  >
                    <Text
                      className="font-medium"
                      style={{ color: timeRange === option.id ? themeColor : colors.text }}
                    >
                      {option.label}
                    </Text>
                  </Pressable>
            ...
            {/* Circle Filter */}
            <View className="mb-6">
              <Text className="text-sm font-medium mb-3" style={{ color: colors.textSecondary }}>FILTER BY CIRCLE</Text>
              {circles.length === 0 ? (
                <View className="p-4 rounded-xl" style={{ backgroundColor: colors.surface }}>
                  <Text style={{ color: colors.textTertiary }} className="text-center">
                    No circles yet. Create one to filter friends.
                  </Text>
                </View>
              ) : (
                <View className="space-y-2">
                  <Pressable
                    ...
                    style={{
                      backgroundColor: !selectedCircleId ? `${themeColor}15` : colors.surface,
                      borderWidth: 1,
                      borderColor: !selectedCircleId ? themeColor : colors.border,
                    }}
                  >
                    <View className="w-8 h-8 rounded-full items-center justify-center mr-3" style={{ backgroundColor: colors.textTertiary + "30" }}>
                      <Users size={16} color={colors.textTertiary} />
                    </View>
                    <Text className="flex-1 font-medium" style={{ color: colors.text }}>All Friends</Text>
                    {!selectedCircleId && <Check size={18} color={themeColor} />}
                  </Pressable>
                  {circles.map((circle) => (
                    <Pressable
                      key={circle.id}
                      ...
                      className="flex-row items-center p-3 rounded-xl"
                      style={{
                        backgroundColor: selectedCircleId === circle.id ? `${themeColor}15` : colors.surface,
                        borderWidth: 1,
                        borderColor: selectedCircleId === circle.id ? themeColor : colors.border,
                      }}
                    >
                      <View
                      ...
                      >
                        <Users size={16} color={circle.color} />
                      </View>
                      <View className="flex-1">
                        <Text className="font-medium" style={{ color: colors.text }}>{circle.name}</Text>
                        <Text className="text-xs" style={{ color: colors.textTertiary }}>
                          {circle.members.length} member{circle.members.length !== 1 ? "s" : ""}
                        </Text>
                      </View>
                      {selectedCircleId === circle.id && <Check size={18} color={themeColor} />}
                    </Pressable>
                  ))}
                </View>
              )}
            ...
            <View className="mb-6">
              <Text className="text-sm font-medium mb-3" style={{ color: colors.textSecondary }}>WORK SCHEDULE</Text>
              <Pressable
                onPress={() => {
              ...
              >
                <View className="flex-row items-center">
                  ...
                  <Text className="font-medium ml-3" style={{ color: colors.text }}>Hide working friends</Text>
                </View>
                <View
                  className="w-6 h-6 rounded-md items-center justify-center"
                  style={{
                    backgroundColor: hideWorking ? themeColor : "transparent",
                    borderWidth: hideWorking ? 0 : 2,
                    borderColor: colors.border,
                  }}
                >
                  {hideWorking && <Check size={14} color="#fff" />}
                </View>
              </Pressable>
            </View>
          </ScrollView>
          ...
          {/* Apply Button */}
          <View className="p-4 border-t" style={{ borderTopColor: colors.separator }}>
            <Pressable
              onPress={() => {
